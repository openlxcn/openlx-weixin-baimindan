...
b=z.read_bytes();m=json.loads((r/'release-manifest.json').read_text());m['versions']['0.1.0']['files']=[{'size':len(b),'sha256':hashlib.sha256(b).hexdigest()}];(out/'manifest.json').write_text(json.dumps(m));print(out)
